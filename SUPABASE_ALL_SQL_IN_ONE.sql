-- ============================================
-- COMPLETE SQL FOR HOTEL PMS SUPABASE SETUP
...
-- ============================================
-- STEP 3: APPLY MIGRATIONS AND UPDATES
-- ============================================

-- Make phone field optional in guests table
ALTER TABLE guests ALTER COLUMN phone DROP NOT NULL;
UPDATE guests SET phone = NULL WHERE phone = '' OR TRIM(phone) = '';

-- Fix room status constraint to include 'dirty' status
ALTER TABLE rooms DROP CONSTRAINT IF EXISTS rooms_status_check;
ALTER TABLE rooms ADD CONSTRAINT rooms_status_check CHECK (status IN ('available', 'occupied', 'maintenance', 'cleaning', 'dirty'));
...
ALTER TABLE notifications ADD COLUMN IF NOT EXISTS priority VARCHAR(20) DEFAULT 'medium' CHECK (priority IN ('low', 'medium', 'high'));
ALTER TABLE notifications DROP CONSTRAINT IF EXISTS notifications_type_check;
ALTER TABLE notifications ADD CONSTRAINT notifications_type_check CHECK (type IN ('booking', 'room', 'housekeeping', 'payment', 'maintenance', 'system', 'check_in', 'check_out'));
...
UPDATE user_settings SET signature = '' WHERE signature IS NULL;

-- Add custom_price column to rooms table
ALTER TABLE rooms ADD COLUMN IF NOT EXISTS custom_price DECIMAL(10, 2);
...
SELECT '✅ Database tables created:' as verification;
SELECT table_name 
...
-- Check sample data
SELECT '✅ Sample users created:' as verification;
SELECT email, first_name, last_name, role FROM users ORDER BY role;

SELECT '✅ Sample rooms created:' as verification;
SELECT room_number, status FROM rooms ORDER BY room_number;

SELECT '✅ Sample bookings created:' as verification;
...
SELECT '🎉 HOTEL PMS DATABASE SETUP COMPLETE!' as message;
SELECT '✅ All tables created' as step_1;
SELECT '✅ Sample data inserted' as step_2;
SELECT '✅ Migrations applied' as step_3;
SELECT '✅ Database ready for Vercel deployment' as step_4;
SELECT ' ' as spacer;
SELECT '📋 NEXT STEPS:' as next_steps;
SELECT '1. Deploy to Vercel' as step;
SELECT '2. Add environment variables (see VERCEL_ENV_VARIABLES.md)' as step;
SELECT '3. Test login with: admin@hotel.com / password123' as step;